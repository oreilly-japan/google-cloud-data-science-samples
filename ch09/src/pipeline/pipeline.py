...
# 8章のパイプラインにエンドポイント作成・モデルデプロイを追加したバージョン
import os
...
pipeline_name = "nyc-taxi-tip-training-and-deployment-pipeline"
...
@kfp.dsl.pipeline(
    name="xgboost-training-and-deployment-pipeline",
    description="XGBoostモデルの学習からデプロイまでを行うパイプライン"
)
def xgboost_training_and_deployment_pipeline(
    project_id: str,
    region: str,
    ...
    endpoint_display_name: str = "nyc-taxi-tip-endpoint",
    ...
    # ===== 8章で作成した部分 =====
    # XGBoostモデルの学習
    train_xgboost_task = train_xgboost(
    ...
    import_model_task = dsl.importer(
        artifact_uri=train_xgboost_task.outputs["model_uri"],
    ...
    # ===== 9章で追加する部分 =====
    # Endpointを作成（学習タスクに依存しないため、学習と並列に実行される）
    ...
    deploy_model_task = ModelDeployOp(
        model=model_upload_task.outputs["model"],  # ModelUploadOpの出力を使用
...
# コンパイル
YAML_FILE_PATH = 'src/pipeline_config/nyc_taxi_tip_training_and_deployment_pipeline.yaml'

if __name__ == "__main__":
    compiler.Compiler().compile(
        pipeline_func=xgboost_training_and_deployment_pipeline,
        package_path=YAML_FILE_PATH
    )
    print(f"Pipeline compiled to {YAML_FILE_PATH}")
