...
INSERT INTO `chatbot.categorized_chat_2`
WITH prompts AS (
  SELECT
  ...
  WHERE DATE(timestamp) = DATE(@run_time, "Asia/Tokyo") - 1
)
SELECT
  timestamp,
  userEmail,
  JSON_VALUE(
    AI.GENERATE(
      prompt => prompt,
      connection_id => "projects/categorize-chat-with-gemini/locations/asia-northeast1/connections/gemini-generate-text",
